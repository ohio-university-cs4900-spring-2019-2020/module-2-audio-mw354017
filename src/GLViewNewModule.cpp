...
void GLViewNewModule::updateWorld()
{
  ...
  //Update the listener's position in the audio engine
   ...
   Aftr::Vector playerPosition(cameraPositionX - 10, cameraPositionY, cameraPositionZ + 10);
   ...
   //starDestroyerDistance += 0.1;
   //Aftr::Vector starDestroyerPosition(starDestroyerDistance, 0, 0);
...
void GLViewNewModule::onKeyDown( const SDL_KeyboardEvent& key )
{
  ...
  if (key.keysym.sym == SDLK_w)
   {
	  	  //irrklang::ISound* starDestroyer = soundEngine->play2D("/sound/xwing", true);
  }
...
void Aftr::GLViewNewModule::loadMap()
{
  ...
  //enemyShip->getModel()->setLookDirection(Aftr::Vector(0, 0, 0));
   ...
   playerShip->setPosition(Vector(0, 0, 0));
